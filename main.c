...
SDL_Texture * createWinTexture(char * text, SDL_Color color){
	...
	else
	{
		...
		if(winTexture == NULL) return NULL;
	}
	SDL_FreeSurface(textSurface);
	return winTexture ;
}
...
int main(int argc, char *args[]) {
	...
	SDL_Texture *winnerTexture = createWinTexture("", scoreColor);
...
//        al_print(objects);
		...
		if (winningTeam > 0)
		{
			winnerTexture = createWinTexture("Winner!", scoreColor);
		}
		else
		{
			winnerTexture = createWinTexture("", scoreColor);
		}
